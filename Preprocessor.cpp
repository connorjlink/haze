...
namespace hz
{
    void Preprocessor::preprocess_include()
    {
        ...
        while (std::regex_search(code, matches, rx_incl))
        {
            ...
            {
                Log::error(fmt::format("file include recursion of {} is unsupported", filepath));
            }
            ...
            if (!file.good())
            {
                Log::error(fmt::format("could not open file {} for reading", filepath));
            }
    ...
    void Preprocessor::preprocess_macro_definition()
    {
        ...
        while (std::regex_search(code, matches, rx_mcro))
        {
            ...
            {
                Log::error(fmt::format("macro {} is multiply defined", macro.name));
            }
    ...
    void Preprocessor::preprocess_macro_invokation()
    {
        ...
        while (std::regex_search(code, matches, rx_invk))
        {
            ...
            {
                Log::error(fmt::format("macro {} is undefined", macro.name));
            }

            if (defined_macro.args.size() != args_delim.size())
            {
                Log::error(fmt::format("invokation of macro {} had {} arguments but expected {}",
