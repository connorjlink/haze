...
namespace hz
{
	...
	public:
		...
		TEST_METHOD(ValidatePointer)
		{
			...
			const auto int_handle = make_handle<IntType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(int_type));
			...
			const auto static_const_unsigned_short_handle = make_handle<IntType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(temp_short));
			...
			const auto extern_const_volatile_long_long_handle = make_handle<IntType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(temp_ll));
		...
		TEST_METHOD(ValidateStruct)
		{
			...
			const auto int_handle = make_handle<IntType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(temp_int));
		...
		TEST_METHOD(ValidateFunction)
		{
			...
			const auto const_char_handle = make_handle<IntType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(temp_char));

			auto temp_enum = EnumType{ .storage = StorageClass::AUTO, .qualifier = TypeQualifier::CONST, .tag = "my_enum" };
			const auto const_enum = make_handle<EnumType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(temp_enum));
			...
			auto case1_helper2 = FunctionType{ .parameters = { make_handle<PointerType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(case1_helper1)) }, .return_type = const_enum, .is_variadic = true };
			const auto case1 = PointerType{ .pointee = make_handle<FunctionType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(case1_helper2)), .qualifier = TypeQualifier::CONST };
		...
		TEST_METHOD(ValidateArray)
		{
			...
			const auto unsigned_short_handle = make_handle<IntType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(temp_short));
			...
			const auto unsigned_short_array_10 = make_handle<ArrayType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(case0));
			...
			const auto case1 = ArrayType{ .element_type = make_handle<PointerType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(case1_helper)), .length = 5 };
			...
			auto case3_func_ptr = PointerType{ .pointee = make_handle<FunctionType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(case3_func)), .qualifier = TypeQualifier::NONE };
			auto case3_helper1 = ArrayType{ .element_type = make_handle<PointerType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(case3_func_ptr)), .length = 5 };
			auto case3_helper2 = PointerType{ .pointee = make_handle<ArrayType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(case3_helper1)), .qualifier = TypeQualifier::NONE };
			const auto case3 = PointerType{ .pointee = make_handle<PointerType, TypeSumDispatcher, TypeSumStorage>(sum, std::move(case3_helper2)), .qualifier = TypeQualifier::NONE };
