...
{ \
	USE_UNSAFE(ErrorReporter)->post_error(parameters.error_frame.context, parameters.error_frame.filepath, \
...
namespace hz
{
	...
	bool TypeValidator::validate_primitive(const Parameters& parameters)
	{
		...
		const auto int_handle = make_handle(parameters.sum, case0);
		...
		const auto static_const_unsigned_short_handle = make_handle(parameters.sum, case1);
		...
		const auto extern_const_volatile_long_long_handle = make_handle(parameters.sum, case2);
		...
		const auto const_char_handle = make_handle(parameters.sum, case3);
	...
	bool TypeValidator::validate_pointer(const Parameters& parameters)
	{
		...
		const auto int_handle = make_handle(parameters.sum, temp_int);
		const auto case0 = PointerType{ .pointee = int_handle, .qualifier = TypeQualifier::NONE };
		VALIDATE("int*", format_type(case0), "int*");

		// const pointer to static const unsigned short
		const auto case1 = Pointer{ .pointee = static_const_unsigned_short_handle, .qualifier = TypeQualifier::CONST };
	...
	bool TypeValidator::validate_enum(const Parameters& parameters)
	{
		...
		const auto const_enum = make_handle(parameters.sum, case0);
	...
	bool TypeValidator::validate_struct(ErrorFrame parameters)
	{
		...
		const auto struct_no_members = make_handle>(sum, case0);
